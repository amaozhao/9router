...
- Docker images now published on both Docker Hub (`decolua/lazirouter`) and GHCR — pull from your preferred registry
...
## Fixes
- DATA_DIR: graceful fallback to ~/.lazirouter on EACCES/EPERM (#1005)
...
## Features
- Speech-to-Text: full pipeline with sttCore + /v1/audio/transcriptions; configs for OpenAI, Gemini, Groq, Deepgram, AssemblyAI, HuggingFace, NVIDIA Parakeet; new lazirouter-stt skill
...
## Fixes
- Fix skills metadata/text in lazirouter, chat, embeddings, image, tts, web-fetch, web-search SKILL.md and skills page
...
- Redirect ~/.lazirouter to DATA_DIR in Docker (persist usage across updates)
