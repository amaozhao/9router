...
async function buildCodexHeader() {
  ...
  const { installed, hasLaziRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasLaziRouter, settings } = result.data;
  ...
  // Extract LaziRouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:LaziRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract LaziRouter provider config
  const provider = settings?.models?.providers?.["lazirouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("lazirouter/") ? primary.replace("lazirouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasLaziRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use LaziRouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
