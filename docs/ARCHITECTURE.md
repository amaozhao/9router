# LaziRouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

LaziRouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[LaziRouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.lazirouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.lazirouter/usage.json`, `~/.lazirouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.lazirouter/db.json`)
- usage stats: `~/.lazirouter/usage.json`
- request log lines: `~/.lazirouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[LaziRouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.lazirouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/lazirouter && npm run build`
- Build Docker image: `cd /root/dev/lazirouter && docker build -t lazirouter .`
