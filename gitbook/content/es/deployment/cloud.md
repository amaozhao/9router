...
Despliega LaziRouter en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.lazirouter` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/lazirouter
sudo chown $USER:$USER /var/lib/lazirouter
```
...
Crea `/etc/nginx/sites-available/lazirouter`:
...
sudo ln -s /etc/nginx/sites-available/lazirouter /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de LaziRouter
...
# Actualizar LaziRouter
cd /path/to/lazirouter/app
git pull
npm install
npm run build
pm2 restart lazirouter
...
tar -czf lazirouter-backup-$(date +%Y%m%d).tar.gz /var/lib/lazirouter

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/lazirouter-$(date +\%Y\%m\%d).tar.gz /var/lib/lazirouter
```
...
```bash
# Verificar si LaziRouter está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/lazirouter
chmod 755 /var/lib/lazirouter
```
