...
リモートアクセスと本番利用のため、VPSまたはDockerにLaziRouterをデプロイ。
...
| `DATA_DIR` | `~/.lazirouter` | データベースとデータの保存パス |
...
`/etc/nginx/sites-available/lazirouter` を作成:
...
# リバースプロキシを使用しない場合、LaziRouterポートを許可
