...
LaziRouterに関する一般的な質問。

---

## LaziRouterとは?

**LaziRouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、LaziRouter自体は100%無料でオープンソースです。**
...
**LaziRouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## LaziRouterをセルフホストできますか?

**はい! LaziRouterは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.lazirouter`)
...
**はい、LaziRouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.lazirouter`(またはカスタム`DATA_DIR`)にローカル保存
- LaziRouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/decolua/lazirouter/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.lazirouter`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/decolua/lazirouter/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [lazirouter.com/docs](https://lazirouter.com/docs)
