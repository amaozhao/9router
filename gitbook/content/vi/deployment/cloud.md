...
Triển khai LaziRouter trên VPS hoặc Docker để truy cập từ xa và dùng trong production.
...
| `DATA_DIR` | `~/.lazirouter` | Đường dẫn lưu database và data |
...
Tạo `/etc/nginx/sites-available/lazirouter`:
...
# If NOT using reverse proxy, allow LaziRouter ports
...
```bash
# Check if LaziRouter is running
pm2 status
