...
Tích hợp LaziRouter với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của LaziRouter.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- LaziRouter đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint LaziRouter:

**Cho LaziRouter cục bộ:**
```
http://localhost:20128/v1
```

**Cho LaziRouter cloud:**
```
https://lazirouter.com
```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint LaziRouter
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key LaziRouter của bạn
2. Bạn có thể tìm API key trong LaziRouter dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-lazirouter-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình LaziRouter
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng LaziRouter để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua LaziRouter
...
3. Nhận giải thích AI qua LaziRouter

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng LaziRouter để hiểu context và thực hiện thay đổi
...
1. Xác minh LaziRouter đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh LaziRouter instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs LaziRouter để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng LaziRouter cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://lazirouter.com`
2. Đảm bảo bạn đã cấu hình API key trong LaziRouter cloud dashboard
...
## Tích hợp với Tính năng LaziRouter

### Định tuyến Model

LaziRouter tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, LaziRouter tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua LaziRouter dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
