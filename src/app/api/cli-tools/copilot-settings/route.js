...
const hasLaziRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "LaziRouter");
};

const getLaziRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "LaziRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getLaziRouterEntry(config);
...
// POST - Apply LaziRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing LaziRouter entry or append
    const idx = config.findIndex((e) => e.name === "LaziRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove LaziRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "LaziRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "LaziRouter removed from Copilot config",
    });
  } catch (error) {
