...
// Check if settings has LaziRouter customModels
const hasLaziRouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:LaziRouter"));
};
...
// POST - Update LaziRouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing LaziRouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:LaziRouter"));
...
// DELETE - Remove LaziRouter customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove LaziRouter customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:LaziRouter"));
