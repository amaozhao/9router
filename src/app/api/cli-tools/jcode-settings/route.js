...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-lazirouter.env");
};
...
const hasLaziRouterConfig = (config) => {
  ...
  if (providers["lazirouter"]) return true;
...
export async function GET() {
  ...
  const hasLaziRouter = hasLaziRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_LAZIROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile lazirouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["lazirouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_LAZIROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "lazirouter configuration removed from jcode",
    });
  } catch (error) {
