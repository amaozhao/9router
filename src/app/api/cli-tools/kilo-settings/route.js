...
const hasLaziRouterConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["lazirouter"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("lazirouter");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasLaziRouter: hasLaziRouterConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "LaziRouter", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "LaziRouter settings removed from Kilo Code" });
  } catch (error) {
