...
// Check if settings has LaziRouter config
const hasLaziRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["lazirouter"];
};

// Read per-agent models.json and return current model id (without "lazirouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["lazirouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasLaziRouter: hasLaziRouterConfig(settings),
...
// POST - Update LaziRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `lazirouter/${model}`;

    // Remove all old lazirouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("lazirouter/"))
    ...
    // Add fresh lazirouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`lazirouter/${m}`] = {};
    });

    // Remove old lazirouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("lazirouter/")) {
    ...
    // Update models.providers.lazirouter with all models
    settings.models.providers["lazirouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `lazirouter/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove LaziRouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove LaziRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["lazirouter"];
    ...
    // Remove lazirouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("lazirouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses lazirouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("lazirouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "LaziRouter settings removed successfully",
    });
  } catch (error) {
