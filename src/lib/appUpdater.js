...
// Collect PIDs of all lazirouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from lazirouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("lazirouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("lazirouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "lazirouter");
  }
  return path.join(os.homedir(), ".lazirouter");
}
...
// Resolve npx/lazirouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx lazirouter` — works regardless of global bin path changes after npm i -g
